...
OpenCode가 에이전트 코딩을 오래 수행하는 동안 Windows가 자동 절전에 들어가지 않도록 유지해, 작업이 중간에 끊기지 않고 끝까지 진행될 수 있게 돕는 백그라운드 프로그램입니다. 작업이 끝나거나 OpenCode가 더 이상 활성 상태가 아니면 절전 방지를 해제하여, Windows가 사용자의 기존 전원/절전 설정을 다시 그대로 따르도록 동작합니다.
